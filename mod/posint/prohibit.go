...
func GetProhibitedItems(Pesan itmodel.IteungMessage, db *mongo.Database) (reply string) {
    ...
    // Check if the first keyword is "list en"
    if len(keywords) > 0 && strings.ToLower(keywords[0]) == "list en" {
        // Remove "list en" from the keywords
        keywords = keywords[1:]
    }

    country, err := GetCountryFromMessage(strings.Join(keywords, " "), db)
    var filter bson.M
    if err != nil {
